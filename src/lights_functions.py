...
class LightMode(Enum):
    ...
    BY_CHANGE = (
        2  # Si fase es 2, rojo y verde duran lo mismo, ambos amarillos duran lo mismo
    )
    BY_COLOR = 3  # Si fase es 3, cada color dura lo suyo, verde, rojo y ambos amarillos
    BY_PHASE = (
        4  # Si fase es 4, cada etapa dura lo suyo, verde, rojo, amarillo 1, amarillo 2
    )
    ...
    def all_lights(self, x):
        ...
        for i in range(tl_num * self.phases):
            ...
            if self.phases == LightMode.SAME_TIME.value:
                tl_dict[id] = [
                    traci.trafficlight.Phase(x[i], p) for p in LightsFunctions.PHASES
                ]  # 0, 1, 2 y 3
                continue
            ...
            if self.phases == LightMode.BY_CHANGE.value:
                tl_dict[id].append(
                    traci.trafficlight.Phase(x[i], LightsFunctions.PHASES[step])
                )  # 0 o 2
                tl_dict[id].append(
                    traci.trafficlight.Phase(x[i], LightsFunctions.PHASES[step + 1])
                )  # 1 o 3
                continue
        ...
        for key in tl_dict:
            ...
            traci.trafficlight.setProgramLogic(
                key, traci.trafficlight.Logic(key, 0, 0, tl_dict[key])
            )

        y = SUMO.run_simulation(self.simulationSteps)["traffic_flow"]
        SUMO.end_simulation()
        if self.show_lights:
            LightsFunctions.__print_lights(tl_dict)
        return 1 - y
