...
def rosenbrock(x, dim):
  fx = 0
  for i in range(dim - 1): fx += 100 * pow(x[i + 1] - x[i] * x[i], 2) + pow(1 - x[i], 2)
  return fx

def rastrigin(x, dim):
  fx = 0
  for i in range(dim): fx += x[i] * x[i] - 10 * cos(2 * pi * x[0])
  return 10 * dim + fx

def styblinski(x, dim):
  fx = 0
  for i in range(dim): fx += pow(x[i], 4) - 16 * pow(x[i], 2) + 5 * x[i]
  return fx / 2
...
def ackley(x, dim):
    ...
    for i in range(dim): 
        sum1 += x[i]**2
        sum2 += cos(2*pi*x[i])
    return -20*pow(e,-0.2*pow(1/dim*sum1,0.5)) - pow(e, 1/dim*sum2) + 20 + e
...
class Particle:
    ...
    def updateParticle(self, Swarm: Swarm):
        ...
        for i in range(Swarm.dim):
            self.v[i] = Swarm.W * self.v[i] + Swarm.C1 * r1 * (self.bestX[i] - self.x[i]) + Swarm.C2 * r2 * (Swarm.BestX[i] - self.x[i])
